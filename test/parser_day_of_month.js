...
test('parse cron with last monday of the month', function (t) {
  var options = {
  ...
  try {
    var interval = CronParser.parseExpression('0 0 0 * * 1L');

    t.equal(interval.hasNext(), true);
    
    var next = interval.next();

    t.equal(next.getDate(), 27);
  } catch (err) {
    t.error(err, 'Parse read error');
  }
...
test('fails to parse for invalid last weekday of month expression', function (t) {
  t.throws(function() {
    CronParser.parseExpression('0 0 0 * * L');
  });
