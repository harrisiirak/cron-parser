...
export class CronExpressionParser {
  ...
  private static constraints: IFieldConstraint[] = Object.values(
    CronConstants.constraints,
  );
  ...
  static parse(
    ...
    const dayOfWeek = CronExpressionParser.#parseField(
      'dayOfWeek',
      _dayOfWeek,
      CronExpressionParser.constraints[5],
    ...
    const fields = new CronFields({
      ...
      dayOfMonth: new CronDayOfMonth(
        dayOfMonth,
        ['*', '?'].includes(rawFields.dayOfMonth),
      ),
      month: new CronMonth(month, ['*', '?'].includes(rawFields.month)),
      dayOfWeek: new CronDayOfTheWeek(
        dayOfWeek,
        ['*', '?'].includes(rawFields.dayOfWeek),
      ),
    });
  ...
  static #parseField(
    ...
    assert(
      CronExpressionParser.validCharacters[field].test(value),
      `Invalid characters, got value: ${value}`,
    );
  ...
  static #parseSequence(
    val: string,
    constraints: IFieldConstraint,
    field: keyof typeof CronExpressionParser.validCharacters,
  ): (number | string)[] {
    ...
    function handleResult(
      ...
      if (Array.isArray(result)) {
        result.forEach((value) => {
          ...
          if (
            !CronExpressionParser.#isValidConstraintChar(constraints, value)
          ) {
    ...
    assert(
      atoms.every((atom) => atom.length > 0),
      'Invalid list value format',
    );
    atoms.forEach((atom) =>
      handleResult(
        CronExpressionParser.#parseRepeat(atom, constraints, field),
        constraints,
      ),
    );
    return stack;
