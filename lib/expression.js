...
CronExpression.dayValidCharactersDayOfMonth = /^[\d|L|/|*|\-|,|\?]+$/;
CronExpression.validCharacters = {
  second: CronExpression.standardValidCharacters,
  minute: CronExpression.standardValidCharacters,
  hour: CronExpression.standardValidCharacters,
  dayOfMonth: CronExpression.dayValidCharactersDayOfMonth,
...
CronExpression._parseField = function _parseField (field, value, constraints) {
  ...
  function parseSequence (val) {
    ...
    if (atoms.length > 1) {
      ...
      if(field === "dayOfMonth") {
        console.log("DoM", JSON.stringify(val));
      }
  ...
  function parseRange (val, repeatInterval) {
    ...
    //edge case, don't covert it to integer
    if(field === "dayOfMonth" && val === "L") {
      return val;
    }
    return +val;
  }
...
CronExpression.prototype._findSchedule = function _findSchedule (reverse) {
  ...
  while (stepCount < LOOP_LIMIT) {
    ...
    if(this._fields.dayOfMonth.length === 1 && this._fields.dayOfMonth[0] === "L") {
      dayOfMonthMatch = currentDate.isLastDayOfMonth();
      // console.log("Check if it's the last day of month.", dayOfMonthMatch);
    }
...
CronExpression.parse = function parse(expression, options, callback) {
  ...
  function parse (expression, options) {
    ...
    if (mappedFields.month.length === 1) {
      ...
      //skip that 
      if(mappedFields.dayOfMonth.length === 1 && mappedFields.dayOfMonth[0] === "L") {
        //nothing
        console.log("Skip")
      } else {
    ...
    console.log("Map:", JSON.stringify(mappedFields))
