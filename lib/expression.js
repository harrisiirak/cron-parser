...
CronExpression._parseField = function _parseField (field, value, constraints) {
  ...
  function parseRange (val, repeatInterval) {
    var stack = [];
    var atoms = val.split('-');
    var hasRepeatStep = typeof repeatInterval !== 'undefined';
    ...
    if (atoms.length > 1) {
      ...
      for (var index = min, count = max; index <= count; index++) {
        if (!hasRepeatStep || (repeatIndex > 0 && (repeatIndex % repeatInterval) === 0)) {
          repeatIndex = 1;
          stack.push(index);
        } else {
          repeatIndex++;
    ...
    if (hasRepeatStep) {
      throw new Error('Invalid value to step, expecting range or sequence');
    }
    return +val;
  }
