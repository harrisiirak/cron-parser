 ...
 * @type {Array}
 */
CronExpression.map = [ 'minute', 'hour', 'dayOfMonth', 'month', 'dayOfWeek', 'year' ];
 ...
 * @type {Array}
 */
CronExpression.parseDefaults = [ '*', '*', '*', '*', '*', (new Date()).getFullYear().toString() ];
...
CronExpression.prototype._findSchedule = function(direction) {
  ...
  while (true) {
    // Match year
    if (!matchSchedule('year', current.getFullYear(), this._fields.year)) {
      current.addYear();
      current.setMonth(0);
      current.setDate(1);
      current.setHours(0);
 ...
 * @public
 */
CronExpression.parse = function(interval, options, callback) {
