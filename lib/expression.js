...
CronExpression.prototype._findSchedule = function _findSchedule () {
  ...
  while (true) {
    ...
    var dayOfMonthMatch = matchSchedule(current.getDate(), this._fields.dayOfMonth);
    var dayOfWeekMatch = matchSchedule(current.getDay(), this._fields.dayOfWeek);
 ...
 * @param {Object} [options] Parsing options
 */
CronExpression.parseSync = function parseSync (expression, options) {
  if (!options) {
    options = {};
  }
