...
CronExpression._sortField = function(field) {
  return field.sort(function(a,b) {
...
CronExpression._handleMaxDaysInMonth = function(mappedFields) {
  ...
  if (mappedFields.month.length === 1) {
    ...
    CronExpression._sortField(mappedFields.dayOfMonth);
  }
};
...
CronExpression.stringify = function stringify(fields, options) {
  function validateConstraints (field, values, constraints, charConstraints) {
    ...
    for (var i = 0, c = values.length; i < c; i++) {
      ...
      // Check constraints
      if (typeof value !== 'number' || value < constraints[0] || value > constraints[1]) {
        throw new Error(
          'Constraint error, got value ' + value + ' expected range ' +
          constraints[0] + '-' + constraints[1]
        );
  ...
  function stringify (fields, options) {
    ...
    for (var i = 0, c = CronExpression.map.length; i < c; ++i) {
      ...
      if (!value) { // Use default value
        ...
        value = CronExpression._sortField(copy)
