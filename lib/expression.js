...
var findRanges = require('./ranges_finder');
...
CronExpression.charConstraints = [
  [], // Second
  [], // Minute
  [], // Hour
  ['L'], // Day of month
 ...
 * @private
 */
CronExpression._parseField = function _parseField (field, value, constraints, charConstraints) {
  // Replace aliases
  switch (field) {
    case 'month':
    case 'dayOfWeek':
    ...
    function handleResult (result) {
      if (result instanceof Array) { // Make sequence linear
        for (var i = 0, c = result.length; i < c; i++) {
          ...
          if (typeof value === 'string' && charConstraints.indexOf(value) > -1) {
            stack.push(value);
            continue;
          }
          // Check constraints
          if (typeof value !== 'number' || safeIsNaN(value) || value < constraints[0] || value > constraints[1]) {
            throw new Error(
        ...
        if (typeof result === 'string' && charConstraints.indexOf(result) > -1) {
          stack.push(result);
          return;
        }
        ...
        // Check constraints
        if (safeIsNaN(numResult) || numResult < constraints[0] || numResult > constraints[1]) {
          throw new Error(
...
CronExpression._handleMaxDaysInMonth = function(mappedFields) {
  ...
  if (mappedFields.month.length === 1) {
    ...
    mappedFields.dayOfMonth.sort(CronExpression._sortCompareFn);
  }
};
 ...
 * @public
 */
CronExpression.prototype.stringify = function stringify () {

  function stringifyField(arr, min, max) {
    var rawRanges = findRanges(arr);
    if (rawRanges.length === 1) {
      var singleRange = rawRanges[0];
    ...
    var result = '';
    for (var i = 0, l = rawRanges.length; i < l; ++i) {
      ...
      if (range.count === 1) {
        ...
        if (step === 1) {
          ...
          result += range.start + '-' + range.end + '/' + step;
        }
      }
      if (i !== l - 1) {
        result += ',';
  ...
  for (var i = 0, c = CronExpression.map.length; i < c; ++i) {
    ...
    result += stringifyField(value, constraint[0], constraint[1]);
    if (i !== c - 1) {
      result += ' ';
    }
  }
...
CronExpression.parse = function parse(expression, options) {
  ...
  function parse (expression, options) {
    ...
    for (var i = 0, c = CronExpression.map.length; i < c; ++i) {
      ...
      if (i < start || !value) { // Use default value
        fields.push(CronExpression._parseField(
          ...
          CronExpression.charConstraints[i]
          )
        );
      } else {
    ...
    CronExpression._handleMaxDaysInMonth(mappedFields);
 ...
 * @return {String}
 */
CronExpression.stringify = function stringify(fields, options) {
  function validateConstraints (field, values, constraints, charConstraints) {
    if (values.length === 0) {
  ...
  function stringify (fields, options) {
    var mappedFields = {};
    for (var i = 0, c = CronExpression.map.length; i < c; ++i) {
      var field = CronExpression.map[i]; // Field name
      var value = fields[field];

      if (!value) { // Use default value
        value = CronExpression._parseField(
          field,
          CronExpression.parseDefaults[i],
          CronExpression.constraints[i],
          CronExpression.charConstraints[i]
        ...
        value = copy.sort(CronExpression._sortCompareFn)
    ...
    var expression = new CronExpression(mappedFields, options || {});
    return expression.stringify();
  }

  return stringify(fields, options);
};
