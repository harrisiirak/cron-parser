...
function CronExpression (fields, options) {
  ...
  this._nthDayOfMonth = options.nthDayOfMonth || 0;
}
...
CronExpression.dayValidCharacters = /^[\d|/|*|\-|,|\?|\L]+$/;
...
CronExpression._parseField = function _parseField (field, value, constraints) {
  ...
  function parseRepeat (val) {
    ...
    if (atoms.length > 1) {
      if(atoms[0].indexOf('-') < 0){
        atoms[0] += '-' + constraints[1]
      }
...
CronExpression.prototype._findSchedule = function _findSchedule (reverse) {
  ...
  function isNthDayMatch(date, nthDayOfWeek) {
    ...
    } else if (nthDayOfWeek === 10) {
      return CronExpression.daysInMonth[date.getMonth()] - date.getDate() < 7
    }
   ...
   * @private
   */
  function isNthDayOfMonthMatch(date, nthDayOfMonth) {
    return CronExpression.daysInMonth[date.getMonth()] - date.getDate() === nthDayOfMonth - 1
  }
  ...
  while (stepCount < LOOP_LIMIT) {
    ...
    if (
      this._nthDayOfMonth > 0 &&
      !isNthDayOfMonthMatch(currentDate, this._nthDayOfMonth)
    ) {
      this._applyTimezoneShift(currentDate, dateMathVerb, 'Day');
      continue;
    }
  ...
  function parse (expression, options) {
    ...
    for (var i = 0, c = CronExpression.map.length; i < c; ++i) {
      ...
      if (i < start || !value) { // Use default value
        ...
        switch (field) {
          case "dayOfWeek": {
            val = parseNthDay(value);
            break;
          }
          case "dayOfMonth": {
            val = parseNthDayOfMonth(value);
            break
          }
        }
    ...
    function parseNthDay(val) {
      ...
      if (atoms.length > 1) {
        ...
        options.nthDayOfWeek = 10;
      }
      return val;
    }

    /**
     * Parses out the # special character for the dayOfMonth field & adds it to options.
     * 
     * @param {String} val 
     * @return {String}
     * @private
     */
    function parseNthDayOfMonth(val) {
      if (val.indexOf("L") > -1){
        if(val === "L"){
          options.nthDayOfMonth = 1
        }else{
          options.nthDayOfMonth = +val.replace("L","");
        }
      }
      return "*";
    }
