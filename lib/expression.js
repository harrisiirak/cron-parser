...
CronExpression._parseField = function _parseField (field, value, constraints) {
  ...
  // check if empty around comma
  if (value === ',') {
    throw new Error('Invalid cron expression, value can not specified as comma');
  }
    ...
    if (atoms.length > 1) {
      for (var i = 0, c = atoms.length; i < c; i++) {
        if(atoms[i]){
          ...
          throw new Error('Invalid cron expression, value can not dangling comma');
        }
      }
    } else {
